...
class Emb_Kmeans_Model:
    ...
    def __init__(
        ...
        self.text_emb = text_cls.agg_doc_rep(
            word2index=word2index, emb_matrix=emb_matrix, agg_func=agg_func
        )
    ...
    def inferences(self):
        df = pd.DataFrame(
            list(zip(self.corpus, self.text_tokenized, self.lengths, self.predict())),
        ...
        fractions = np.sort(
            np.array(cluster_doc_count) * 100.0 / sum(cluster_doc_count)
        )
        ...
        for c in populars:
            freq_dist = dict(
                FreqDist(
                    [
                        token
                        for doc in df.Tokenized[df.Class == c].to_list()
                        for token in doc
                    ]
                )
            )
...
if __name__ == "__main__":
    print(
        f"This code contains Emb_Kmeans class and its associated functions to work with Sentence Embedding + K-means topic modeling!"
    )
