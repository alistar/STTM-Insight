...
class GSDMM_Model:
    ...
    def __init__(
        ...
        if verbose:
            print(
                f"GSDMS model parameters: n_class = {n_class}, alpha = {alpha}, beta = {beta}, n_iters = {n_iters}.\n"
            )
        self.model = MovieGroupProcess(
            K=n_class, alpha=alpha, beta=beta, n_iters=n_iter
        )
    ...
    def predict(self, new_data=None):
        if new_data:
            ...
            return [
                self.model.choose_best_label(sent)[0] for sent in self.text_tokenized
            ]

    def hcv_score(self, new_data=None, true_labels=None):
        return homogeneity_completeness_v_measure(
            true_labels, self.predict(new_data=new_data)
        )
    ...
    def inferences(self):
        ...
        fractions = np.sort(
            np.array(self.model.cluster_doc_count)
            * 100.0
            / sum(self.model.cluster_doc_count)
        )
        ...
        for c in range(self.n_class):
            freq_dists.append(
                dict(
                    sorted(
                        model.cluster_word_distribution[populars[c]].items(),
...
if __name__ == "__main__":
    print(
        f"This code contains a GSDM class and its associated functions to work with GSDM topic modeling!"
    )
